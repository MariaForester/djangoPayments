...
class UserPersonalUseView(views.APIView):
    ...
    1 - no wallets for this user yet.
    """

    def get(self, request):
        wallets = list(Wallets.objects.filter(user_id_id=request.user.id).values('wallet_num', 'wallet_sum'))
        if not wallets:
            return JsonResponse({
                'code': 1,
            ...
            return JsonResponse({
                'code': 0,
                'message': wallet_serializer.initial_data
            })
